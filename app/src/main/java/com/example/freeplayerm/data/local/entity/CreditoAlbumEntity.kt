 ...
 *
 * Los álbumes tienen múltiples personas involucradas en su creación más allá del artista.
 * Esta entidad permite registrar a todos los contribuyentes.
 *
 * Relaciones:
 * - N:1 con AlbumEntity (un álbum tiene múltiples créditos)
 * - Opcionalmente relacionado con ArtistaEntity si el acreditado también es artista
 *
 * Casos de uso:
...
data class CreditoAlbumEntity(
    ...
    /**
     * ID del artista si esta persona también es un artista registrado
     * Permite vincular créditos con perfiles de artista
     */
    ...
    /**
     * Rol principal en la producción del álbum
     * Valores: PRODUCER, EXECUTIVE_PRODUCER, ENGINEER, MIXING_ENGINEER,
     *          MASTERING_ENGINEER, COMPOSER, SONGWRITER, ARRANGER,
     *          MUSICIAN, VOCALIST, FEATURED_ARTIST, GUEST, etc.
     */
    @ColumnInfo(name = "rol")
    val rol: String,

    /**
     * Roles adicionales que desempeñó (JSON array)
     * Ejemplo: ["engineer", "mixing"]
     */
    @ColumnInfo(name = "roles_adicionales_json")
    val rolesAdicionalesJson: String? = null,

    /**
     * Instrumentos que tocó (si es músico) - JSON array
     * Ejemplo: ["guitar", "bass", "keyboards"]
     */
    @ColumnInfo(name = "instrumentos_json")
    val instrumentosJson: String? = null,

    /**
     * Tracks específicos en los que trabajó (JSON array de números)
     * Si es null, significa que trabajó en todo el álbum
     * Ejemplo: [1,3,5,7] = trabajó en los tracks 1, 3, 5 y 7
     */
    @ColumnInfo(name = "tracks_json")
    val tracksJson: String? = null,

    // ==================== VISUALIZACIÓN ====================

    /**
     * Orden de visualización dentro de su categoría de rol
     * Menor = más importante (1 = primero)
     */
    @ColumnInfo(name = "orden")
    val orden: Int = 999,

    /**
     * Si debe mostrarse destacado en la lista de créditos
     * Ejemplo: productor ejecutivo, productor principal
     */
     ...
     * Valores: "MANUAL", "MUSICBRAINZ", "DISCOGS", "ALLMUSIC", "GENIUS"
     */
    ...
    companion object {
        ...
        object Rol {
            ...
            fun obtenerCategoria(rol: String): String {
                return when (rol) {
                    PRODUCER, EXECUTIVE_PRODUCER, CO_PRODUCER, ASSOCIATE_PRODUCER -> "Producción"
                    ENGINEER, MIXING_ENGINEER, MASTERING_ENGINEER, RECORDING_ENGINEER,
                    ASSISTANT_ENGINEER, MIXER -> "Ingeniería"
                    COMPOSER, SONGWRITER, LYRICIST, ARRANGER -> "Composición"
                    MUSICIAN, SESSION_MUSICIAN -> "Instrumentistas"
                    VOCALIST, BACKGROUND_VOCALIST -> "Voces"
                    FEATURED_ARTIST, GUEST_ARTIST -> "Artistas Invitados"
                    ART_DIRECTION, PHOTOGRAPHY, GRAPHIC_DESIGN -> "Arte y Diseño"
                    else -> "Otros"
                }
            }
        ...
        fun crearProductor(
            ...
            return CreditoAlbumEntity(
                idAlbum = idAlbum,
                nombre = nombre,
                rol = if (ejecutivo) Rol.EXECUTIVE_PRODUCER else Rol.PRODUCER,
                destacado = destacado,
                orden = Rol.obtenerPrioridad(if (ejecutivo) Rol.EXECUTIVE_PRODUCER else Rol.PRODUCER)
            )
        }
    ...
    fun esRolProduccion(): Boolean {
        return rol in listOf(
            Rol.PRODUCER,
            Rol.EXECUTIVE_PRODUCER,
            Rol.CO_PRODUCER,
            Rol.ASSOCIATE_PRODUCER
        )
    }
