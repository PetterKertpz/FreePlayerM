 ...
 *
 * ⚠️ ADVERTENCIA: Esta entidad genera muchos registros y puede afectar performance. Solo usar si
 * necesitas:
 ...
 *
 * Relaciones:
 * - N:1 con PlaylistEntity
 * - N:1 con UserEntity (quien hizo el cambio)
 *
...
data class PlaylistOrderHistoryEntity(
    ...
    /** Usuario que realizó el cambio Puede ser null si fue una operación del sistema */
    @ColumnInfo(name = "id_usuario") val idUsuario: Int? = null,

    // ==================== OPERACIÓN ====================

    /**
     * Tipo de operación realizada Valores: REORDER, ADD, REMOVE, MOVE, BATCH_REORDER, SHUFFLE, SORT
     */
    @ColumnInfo(name = "tipo_operacion") val tipoOperacion: String,

    /** ID de la canción afectada (null para operaciones batch) */
    @ColumnInfo(name = "id_cancion") val idCancion: Int? = null,

    // ==================== CAMBIO DE POSICIÓN ====================

    /** Posición anterior (orden antes del cambio) null para operaciones ADD */
    @ColumnInfo(name = "posicion_anterior") val posicionAnterior: Int? = null,

    /** Posición nueva (orden después del cambio) null para operaciones REMOVE */
    @ColumnInfo(name = "posicion_nueva") val posicionNueva: Int? = null,
     ...
     *
     * Para BATCH_REORDER: {"changes": [{"id":1,"from":5,"to":2}]} Para SHUFFLE: {"seed": 12345,
     * "count": 50} Para SORT: {"criteria": "title", "direction": "asc"}
     */
    @ColumnInfo(name = "detalles_json") val detallesJson: String? = null,
    ...
    @ColumnInfo(name = "reversible") val reversible: Boolean = true,

    /** Si ya fue revertida */
    @ColumnInfo(name = "revertida") val revertida: Boolean = false,

    /** ID del historial que revirtió esta operación (si aplica) */
    @ColumnInfo(name = "revertida_por") val revertidaPor: Int? = null,
) {
    companion object {
         ...
         *
         * Para evitar que la tabla crezca infinitamente, se recomienda mantener solo los últimos N
         * días
         */
        const val DIAS_RETENCION_DEFAULT = 30

        /** Máximo de registros por playlist Después de este límite, eliminar los más antiguos */
        ...
        fun crearMovimiento(
            ...
            posicionNueva: Int,
        ): PlaylistOrderHistoryEntity {
            return PlaylistOrderHistoryEntity(
                idLista = idLista,
                idUsuario = idUsuario,
    ...
    fun marcarComoRevertida(idHistorialReversion: Int): PlaylistOrderHistoryEntity {
        return copy(revertida = true, revertidaPor = idHistorialReversion)
    }

    /** Crea la operación inversa para deshacer */
    fun crearOperacionInversa(): PlaylistOrderHistoryEntity? {
        if (!puedeDeshacer()) return null

        return when (tipoOperacion) {
            TipoOperacion.MOVE,
            TipoOperacion.REORDER -> {
