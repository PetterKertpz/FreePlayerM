...
interface PlaybackQueueDao {
    ...
    suspend fun insertarEnCola(item: PlaybackQueueEntity): Long

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertarEnCola(items: List<PlaybackQueueEntity>): List<Long>

    // ==================== ACTUALIZACIÓN ====================

    @Update suspend fun actualizarItem(item: PlaybackQueueEntity): Int
    ...
    @Delete suspend fun eliminarItem(item: PlaybackQueueEntity): Int
    ...
    suspend fun obtenerItemPorId(id: Int): PlaybackQueueEntity?
    ...
    fun obtenerColaCompleta(usuarioId: Int): Flow<List<PlaybackQueueEntity>>
    ...
    suspend fun obtenerColaCompletaSync(usuarioId: Int): List<PlaybackQueueEntity>
    ...
    fun obtenerColaPendiente(usuarioId: Int): Flow<List<PlaybackQueueEntity>>
    ...
    suspend fun obtenerColaPendienteSync(usuarioId: Int): List<PlaybackQueueEntity>
    ...
    fun obtenerCancionesEnColaConDetalles(usuarioId: Int): Flow<List<SongWithArtist>>
    ...
    suspend fun obtenerSiguienteEnCola(usuarioId: Int): PlaybackQueueEntity?
    ...
    suspend fun obtenerSiguienteDespuesDe(usuarioId: Int, ordenActual: Int): PlaybackQueueEntity?
    ...
    suspend fun obtenerAnteriorAnteDe(usuarioId: Int, ordenActual: Int): PlaybackQueueEntity?
    ...
    fun obtenerColaPorOrigen(usuarioId: Int, origen: String): Flow<List<PlaybackQueueEntity>>
    ...
    fun obtenerColaManuales(usuarioId: Int): Flow<List<PlaybackQueueEntity>>
    ...
    fun obtenerColaSugerencias(usuarioId: Int): Flow<List<PlaybackQueueEntity>>
    ...
    suspend fun reordenarCola(usuarioId: Int, nuevosOrdenes: Map<Int, Int>) {
        nuevosOrdenes.forEach { (idCola, orden) -> actualizarOrden(idCola, orden) }
    }
    ...
    suspend fun moverItem(usuarioId: Int, idCola: Int, nuevaPosicion: Int) {
        ...
        items.forEachIndexed { index, itemCola -> actualizarOrden(itemCola.idCola, index) }
    }
    ...
    suspend fun agregarSiguiente(
        ...
        items
            .filter { !it.reproducido }
            .forEach { item -> actualizarOrden(item.idCola, item.orden + 1) }
    ...
    @Transaction
    suspend fun reproducirSiguiente(usuarioId: Int): PlaybackQueueEntity? {
        val siguiente = obtenerSiguienteEnCola(usuarioId)
        siguiente?.let { marcarComoReproducido(it.idCola, true) }
        return siguiente
    }
    ...
    suspend fun marcarTodasComoReproducidas(usuarioId: Int) {
        ...
        items.forEach { item -> marcarComoReproducido(item.idCola, true) }
    }

    @Transaction
    suspend fun desmarcarTodasComoReproducidas(usuarioId: Int) {
        val items = obtenerColaCompletaSync(usuarioId)
        items.forEach { item -> marcarComoReproducido(item.idCola, false) }
    }
    ...
    suspend fun compactarOrdenes(usuarioId: Int) {
        val items = obtenerColaCompletaSync(usuarioId).sortedBy { it.orden }

        items.forEachIndexed { index, item -> actualizarOrden(item.idCola, index) }
    }
    ...
    suspend fun obtenerItemAleatorio(usuarioId: Int): PlaybackQueueEntity?
