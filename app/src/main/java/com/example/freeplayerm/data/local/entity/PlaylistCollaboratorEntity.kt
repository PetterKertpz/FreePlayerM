 ...
 *
 * Reemplaza el campo booleano simple "esColaborativa" de PlaylistEntity con un sistema de permisos
 * detallado por usuario.
 *
 * Relaciones:
 * - N:1 con PlaylistEntity (una playlist tiene múltiples colaboradores)
 * - N:1 con UserEntity (un usuario puede colaborar en múltiples playlists)
 *
 * Casos de uso:
...
data class PlaylistCollaboratorEntity(
    ...
    /**
     * Rol del colaborador en la playlist Valores: OWNER, EDITOR, CONTRIBUTOR, VIEWER
    ...
    @ColumnInfo(name = "puede_agregar") val puedeAgregar: Boolean = false,

    /** Puede eliminar canciones de la playlist */
    @ColumnInfo(name = "puede_eliminar") val puedeEliminar: Boolean = false,

    /** Puede reordenar canciones existentes */
    @ColumnInfo(name = "puede_reordenar") val puedeReordenar: Boolean = false,

    /** Puede editar título, descripción y portada */
    @ColumnInfo(name = "puede_editar_metadata") val puedeEditarMetadata: Boolean = false,

    /** Puede invitar a otros colaboradores */
    @ColumnInfo(name = "puede_invitar") val puedeInvitar: Boolean = false,
    ...
    @ColumnInfo(name = "puede_cambiar_visibilidad") val puedeCambiarVisibilidad: Boolean = false,

    // ==================== INVITACIÓN Y ESTADO ====================

    /** ID del usuario que invitó a este colaborador */
    @ColumnInfo(name = "invitado_por") val invitadoPor: Int? = null,
    @ColumnInfo(name = "fecha_invitacion") val fechaInvitacion: Long = System.currentTimeMillis(),

    /** Fecha en que aceptó la invitación (null si aún no acepta) */
    @ColumnInfo(name = "fecha_aceptacion") val fechaAceptacion: Int? = null,

    /** Estado de la colaboración Valores: PENDING, ACTIVE, REJECTED, REMOVED */
    @ColumnInfo(name = "estado") val estado: String = "PENDING",

    // ==================== ESTADÍSTICAS ====================

    /** Cantidad de canciones agregadas por este colaborador */
    @ColumnInfo(name = "canciones_agregadas") val cancionesAgregadas: Int = 0,

    /** Cantidad de cambios realizados (ediciones, reorden, etc.) */
    @ColumnInfo(name = "cambios_realizados") val cambiosRealizados: Int = 0,

    /** Última actividad en la playlist */
    @ColumnInfo(name = "ultima_actividad") val ultimaActividad: Int? = null,

    // ==================== METADATOS ====================

    /** Mensaje de invitación personalizado */
    @ColumnInfo(name = "mensaje_invitacion") val mensajeInvitacion: String? = null,
    ...
    companion object {
        ...
        fun crearOwner(idLista: Int, idUsuario: Int): PlaylistCollaboratorEntity {
    ...
    fun aceptarInvitacion(): PlaylistCollaboratorEntity {
        return copy(estado = Estado.ACTIVE, fechaAceptacion = System.currentTimeMillis().toInt())
    }

    /** Copia rechazando la invitación */
    fun rechazarInvitacion(): PlaylistCollaboratorEntity {
        return copy(estado = Estado.REJECTED)
    }

    /** Copia removiendo al colaborador */
    fun remover(): PlaylistCollaboratorEntity {
        return copy(estado = Estado.REMOVED, activo = false)
    }

    /** Copia incrementando contador de cambios */
    fun registrarCambio(): PlaylistCollaboratorEntity {
        return copy(
    ...
    fun registrarCancionAgregada(): PlaylistCollaboratorEntity {
        return copy(
    ...
    fun cambiarRol(nuevoRol: String): PlaylistCollaboratorEntity {
        if (esOwner()) {
