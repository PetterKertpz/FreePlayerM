// en: app/src/main/java/com/example/freeplayerm/data/local/entity/HistorialReproduccionEntity.kt
 ...
 *
 * Entidad que registra cada reproducción de una canción
 * Permite analytics detallado, recomendaciones y estadísticas
 *
 * Características:
...
@Entity(
    ...
    indices = [
        ...
        Index(value = ["id_usuario", "fecha_reproduccion"]), // Para queries de historial por usuario
...
data class HistorialReproduccionEntity(
    ...
    @ColumnInfo(name = "porcentaje_reproducido")
    val porcentajeReproducido: Float = 0f, // 0.0 - 1.0
    ...
    val origen: String, // "PLAYLIST", "ALBUM", "ARTISTA", "GENERO", "BUSQUEDA", "ALEATORIO", "FAVORITOS", "RECOMENDACION"
    ...
    val volumen: Float? = null, // 0.0 - 1.0

    @ColumnInfo(name = "con_ecualizador")
    val conEcualizador: Boolean = false,
    ...
    @ColumnInfo(name = "agregado_a_playlist_durante")
    val agregadoAPlaylistDurante: Boolean = false,

    // ==================== METADATA ADICIONAL ====================

    @ColumnInfo(name = "hora_del_dia")
    val horaDelDia: Int = 0, // 0-23, útil para detectar patrones

    @ColumnInfo(name = "dia_semana")
    val diaSemana: Int = 0, // 1-7 (1=Lunes, 7=Domingo)
     ...
     * Basado en: reproducción completa, pausas, seeks, favoritos
     */
    fun calcularEngagementScore(): Int {
        var score = 0

        // Reproducción completa +40 puntos
        if (completo) score += 40
        else score += (porcentajeReproducido * 40).toInt()
    ...
    companion object {
        ...
        fun crearCompleto(
            ...
            val basico = crear(idUsuario, idCancion, duracionReproducidaMs, duracionTotalMs, origen, idContexto)
