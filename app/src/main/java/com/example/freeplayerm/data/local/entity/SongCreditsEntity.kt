 ...
 * 🎼 SONG CREDITS ENTITY
 *
 * Almacena créditos de canciones (productores, compositores, ingenieros, etc.) Fuente: Genius API
 * CustomPerformances
 */
@Entity(
    ...
    indices = [Index(value = ["id_cancion"]), Index(value = ["id_artista"]), Index(value = ["rol"])],
)
data class SongCreditsEntity(
    ...
    @ColumnInfo(name = "nombre_persona") val nombrePersona: String, // Si no hay artista asociado
    @ColumnInfo(name = "rol") val rol: String, // PRODUCER, WRITER, ENGINEER, MIXER, etc.
    @ColumnInfo(name = "orden") val orden: Int = 0,
    @ColumnInfo(name = "fuente") val fuente: String = "GENIUS",
