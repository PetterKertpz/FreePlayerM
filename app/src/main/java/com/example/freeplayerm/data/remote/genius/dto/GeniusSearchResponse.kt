 ...
 *
 * DTO para respuesta del endpoint /search de Genius API Estructura: { meta: {...}, response: {
 * hits: [...] } }
 */
...
@JsonClass(generateAdapter = true)
data class SearchResponseData(@Json(name = "hits") val hits: List<Hit>? = null)
...
@JsonClass(generateAdapter = true) data class MetaInfo(@Json(name = "status") val status: Int)

// ==================== EXTENSION FUNCTIONS ====================

/** Valida que el resultado sea una canción válida */
fun SongResult.isValid(): Boolean {
    return id.isNotBlank() && title.isNotBlank() && url.isNotBlank() && primaryArtist != null
}
