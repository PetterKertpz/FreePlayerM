// en: app/src/main/java/com/example/freeplayerm/data/local/entity/PlaybackQueueEntity.kt
 ...
 *
 * Entidad que representa la cola de reproducción dinámica de cada usuario Permite agregar canciones
 * "a continuación" o "al final de la cola"
 *
 * Características:
 ...
 *
 * Diferencia con PlaybackStateEntity:
...
data class PlaybackQueueEntity(
    @PrimaryKey(autoGenerate = true) @ColumnInfo(name = "id_cola") val idCola: Int = 0,
    ...
    @ColumnInfo(name = "reproducido") val reproducido: Boolean = false, // Si ya se reprodujo
    @ColumnInfo(name = "fecha_reproducido") val fechaReproducido: Long? = null,

    // ==================== ORIGEN ====================

    @ColumnInfo(name = "origen") val origen: String, // "MANUAL", "SUGERENCIA", "RADIO", "AUTOPLAY"
    ...
    val razonSugerencia: String? =
        null, // Por qué se sugirió ("similar_genre", "same_artist", etc.)
    ...
    fun marcarComoReproducida(): PlaybackQueueEntity {
        return copy(reproducido = true, fechaReproducido = System.currentTimeMillis())
    }
    ...
    companion object {
        ...
        const val RAZON_FRECUENTEMENTE_JUNTAS =
            "frequently_together" // Se reproducen juntas a menudo
        ...
        fun crearAutoplay(idUsuario: Int, idCancion: Int, orden: Int): PlaybackQueueEntity {
            return PlaybackQueueEntity(
                idUsuario = idUsuario,
                idCancion = idCancion,
                orden = orden,
        ...
        fun crearConCrossfade(
            ...
            return crearManual(idUsuario, idCancion, orden)
                .copy(
        ...
        /** Reordena una lista de entradas de cola Actualiza el campo 'orden' de cada una */
        fun reordenar(items: List<PlaybackQueueEntity>): List<PlaybackQueueEntity> {
            return items.mapIndexed { index, item -> item.copy(orden = index) }
        }

        /**
         * Inserta una canción en una posición específica Retorna la lista actualizada con órdenes
         * correctos
         */
        fun insertarEn(
            lista: List<PlaybackQueueEntity>,
            nuevaEntrada: PlaybackQueueEntity,
            posicion: Int,
        ): List<PlaybackQueueEntity> {
