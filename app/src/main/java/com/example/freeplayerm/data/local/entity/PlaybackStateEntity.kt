// en: app/src/main/java/com/example/freeplayerm/data/local/entity/PlaybackStateEntity.kt
 ...
 *
 * Entidad singleton (1 por usuario) que almacena el estado actual del reproductor Permite retomar
 * la reproducción exactamente donde el usuario la dejó
 *
 * Características:
...
data class PlaybackStateEntity(
    ...
    @ColumnInfo(name = "id_cancion_actual") val idCancionActual: Int? = null,
    @ColumnInfo(name = "posicion_ms") val posicionMs: Long = 0, // Posición actual en milisegundos
    @ColumnInfo(name = "duracion_cancion_ms")
    val duracionCancionMs: Long = 0L, // Duración total de la canción

    // ==================== ESTADO DE REPRODUCCIÓN ====================

    @ColumnInfo(name = "esta_reproduciendo") val estaReproduciendo: Boolean = false,
    @ColumnInfo(name = "velocidad_reproduccion")
    val velocidadReproduccion: Float = 1.0f, // 0.5x - 2.0x
    @ColumnInfo(name = "pitch") val pitch: Float = 1.0f, // Tono de la música

    // ==================== MODOS DE REPRODUCCIÓN ====================

    @ColumnInfo(name = "modo_repetir")
    val modoRepetir: String = MODO_REPETIR_NINGUNO, // "NONE", "ONE", "ALL"
    @ColumnInfo(name = "modo_aleatorio") val modoAleatorio: Boolean = false,
    @ColumnInfo(name = "modo_aleatorio_inteligente")
    val modoAleatorioInteligente: Boolean = false, // Shuffle basado en gustos

    // ==================== AUDIO ====================

    @ColumnInfo(name = "volumen") val volumen: Float = 0.7f, // 0.0 - 1.0
    @ColumnInfo(name = "silenciado") val silenciado: Boolean = false,
    @ColumnInfo(name = "ecualizador_activo") val ecualizadorActivo: Boolean = false,
    @ColumnInfo(name = "ecualizador_preset") val ecualizadorPreset: String? = null,

    // ==================== CONTEXTO DE REPRODUCCIÓN ====================

    @ColumnInfo(name = "tipo_contexto")
    val tipoContexto: String? =
        null, // "PLAYLIST", "ALBUM", "ARTISTA", "GENERO", "FAVORITOS", "ALEATORIO"
    @ColumnInfo(name = "id_contexto") val idContexto: Int? = null, // ID de la playlist/álbum/etc
    ...
    @ColumnInfo(name = "indice_cola_actual") val indiceColaActual: Int = 0, // Posición en la cola
    @ColumnInfo(name = "total_canciones_cola") val totalCancionesCola: Int = 0,
    @ColumnInfo(name = "cola_ids_json")
    val colaIdsJson: String? = null, // Array de IDs de canciones en cola (JSON)
    @ColumnInfo(name = "cola_origen") val colaOrigen: String? = null, // De dónde se generó la cola
    ...
    @ColumnInfo(name = "puede_ir_anterior") val puedeIrAnterior: Boolean = false,
    @ColumnInfo(name = "puede_ir_siguiente") val puedeIrSiguiente: Boolean = false,
    ...
    fun conPosicionActualizada(nuevaPosicionMs: Long): PlaybackStateEntity {
        return copy(
    ...
    companion object {
        ...
        fun crearVacio(idUsuario: Int): PlaybackStateEntity {
            return PlaybackStateEntity(idUsuario = idUsuario)
        }
