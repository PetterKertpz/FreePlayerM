// en: app/src/main/java/com/example/freeplayerm/data/local/dao/LyricsDao.kt
...
interface LyricsDao {
    ...
    /** Inserta o actualiza una letra Si ya existe para esa canción, la reemplaza */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertarLetra(letra: LyricsEntity): Long

    /** Inserta múltiples letras en una sola transacción */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertarLetras(letras: List<LyricsEntity>): List<Long>

    /** Actualiza una letra existente */
    @Update suspend fun actualizarLetra(letra: LyricsEntity): Int

    /** Elimina una letra */
    @Delete suspend fun eliminarLetra(letra: LyricsEntity): Int
    ...
    /**
     * Obtiene la letra de una canción específica (Flow reactivo) Se actualiza automáticamente
     * cuando cambia en la BD
     */
    @Query("SELECT * FROM letras WHERE id_cancion = :idCancion")
    fun obtenerLetraPorIdCancion(idCancion: Int): Flow<LyricsEntity?>

    /**
     * Obtiene la letra de una canción específica (suspending) Para operaciones puntuales sin
     * reactividad
     */
    @Query("SELECT * FROM letras WHERE id_cancion = :idCancion")
    suspend fun obtenerLetraPorIdCancionSuspending(idCancion: Int): LyricsEntity?

    /** Obtiene todas las letras almacenadas */
    @Query("SELECT * FROM letras ORDER BY id_cancion ASC")
    fun obtenerTodasLasLetras(): Flow<List<LyricsEntity>>

    /** Obtiene todas las letras (suspending) */
    @Query("SELECT * FROM letras ORDER BY id_cancion ASC")
    suspend fun obtenerTodasLasLetrasSuspending(): List<LyricsEntity>

    // ==================== BÚSQUEDA Y FILTROS ====================

    /** Busca letras por contenido de texto Útil para encontrar canciones por fragmentos de letra */
    @Query(
        """
        SELECT * FROM letras 
    ...
    suspend fun buscarLetrasPorContenido(query: String, limit: Int = 50): List<LyricsEntity>
    ...
    fun obtenerLetrasPorFuente(fuente: String): Flow<List<LyricsEntity>>
    ...
    fun obtenerLetrasRecientes(limit: Int = 20): Flow<List<LyricsEntity>>

    // ==================== ESTADÍSTICAS Y CONTADORES ====================

    /** Cuenta cuántas letras están almacenadas */
    @Query("SELECT COUNT(*) FROM letras") suspend fun contarLetras(): Int
    ...
    /** Obtiene letras que necesitan actualización (por ejemplo, más antiguas que cierta fecha) */
    @Query(
        """
        SELECT * FROM letras 
    ...
    /** Elimina todas las letras ⚠️ Usar con precaución */
    @Query("DELETE FROM letras") suspend fun eliminarTodasLasLetras(): Int

    // ==================== OPERACIONES AVANZADAS ====================

    /** Actualiza o inserta una letra con validación */
    @Transaction
    suspend fun upsertLetra(
        idCancion: Int,
        textoLetra: String,
        fuente: String = "manual",
    ): Long { // ✅ Añadir tipo de retorno Long
        val letraExistente = obtenerLetraPorIdCancionSuspending(idCancion)

        return if (letraExistente != null) {
            val letraActualizada =
                letraExistente.copy(
            ...
            val nuevaLetra =
                LyricsEntity(idCancion = idCancion, textoLetra = textoLetra, fuente = fuente)
...
"""
    ...
    /** Busca canciones sin letra (canciones que existen pero no tienen letra asociada) */
    @Query(
        """
        SELECT c.id_cancion 
        FROM canciones c 
