      ...
      return try {
         ...
         when {
            ...
            mimeType?.contains("webp") == true -> "webp"
               ...
               uri.path?.substringAfterLast('.')?.lowercase()?.takeIf { it in ALLOWED_EXTENSIONS }
            }
         }
      } catch (e: Exception) {
   ...
   suspend fun getAlbumArtUri(albumId: Long): Uri? {
      return withContext(Dispatchers.IO) {
         try {
            ...
            val isValid = context.contentResolver.openInputStream(albumArtUri)?.use {
               it.read() != -1
            } ?: false
            
            if (isValid) {
               Log.d(tag, "✅ Portada encontrada para álbum: $albumId")
               albumArtUri
            } else {
               Log.d(tag, "📷 Sin portada para álbum: $albumId")
               null
            }
         } catch (e: Exception) {
            Log.w(tag, "⚠️ Error obteniendo portada de álbum $albumId: ${e.message}")
            null
         }
      }
   }
