...
/**
 * DAO para operaciones de playlists en la base de datos. ✅ CORREGIDO - Campos sincronizados con
 * PlaylistEntity
 */
@Dao
interface PlaylistDao {
    ...
    suspend fun insertarLista(lista: PlaylistEntity): Long

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertarListas(listas: List<PlaylistEntity>): List<Long>

    // ==================== ACTUALIZACIÓN ====================

    @Update suspend fun actualizarLista(lista: PlaylistEntity): Int
    ...
    @Delete suspend fun eliminarLista(lista: PlaylistEntity): Int

    @Query("DELETE FROM listas_reproduccion WHERE id_lista = :id")
    suspend fun eliminarListaPorId(id: Int): Int

    @Query(
        "DELETE FROM listas_reproduccion WHERE total_canciones = 0 AND fecha_creacion < :timestampLimite"
    )
    ...
    suspend fun obtenerListaPorId(id: Int): PlaylistEntity?

    @Query("SELECT * FROM listas_reproduccion WHERE id_lista = :id LIMIT 1")
    fun obtenerListaPorIdFlow(id: Int): Flow<PlaylistEntity?>

    @Query("SELECT * FROM listas_reproduccion ORDER BY nombre ASC")
    fun obtenerTodasLasListas(): Flow<List<PlaylistEntity>>

    @Query("SELECT COUNT(*) FROM listas_reproduccion") suspend fun contarListas(): Int

    @Query("SELECT COUNT(*) FROM listas_reproduccion") fun contarListasFlow(): Flow<Int>
    ...
    fun buscarListas(query: String, limite: Int = 50): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasDelUsuario(usuarioId: Int): Flow<List<PlaylistEntity>>
    ...
    suspend fun obtenerListasDelUsuarioSync(usuarioId: Int): List<PlaylistEntity>
    ...
    fun obtenerListasPublicas(limite: Int = 50): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasPrivadasDelUsuario(usuarioId: Int): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasColaborativas(usuarioId: Int): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasRecientes(usuarioId: Int, limite: Int = 20): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasMasEscuchadas(usuarioId: Int, limite: Int = 20): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasMasGrandes(usuarioId: Int, limite: Int = 20): Flow<List<PlaylistEntity>>
    ...
    fun obtenerListasReproducidasRecientemente(
        usuarioId: Int,
        limite: Int = 10,
    ...
    fun obtenerListasPublicasPopulares(limite: Int = 50): Flow<List<PlaylistEntity>>
    ...
    @Transaction
    suspend fun obtenerOCrearLista(nombre: String, usuarioId: Int): PlaylistEntity {
        ...
        val nuevaLista = PlaylistEntity(nombre = nombre, idUsuario = usuarioId)
