...
/**
 * DAO para operaciones de álbumes en la base de datos. ✅ CORREGIDO - Campos sincronizados con
 * AlbumEntity
 */
@Dao
interface AlbumDao {
    ...
    @Query("DELETE FROM albumes WHERE id_album = :id") suspend fun eliminarAlbumPorId(id: Int): Int

    @Query("DELETE FROM albumes WHERE total_canciones = 0") suspend fun eliminarAlbumesVacios(): Int
    ...
    @Query("SELECT COUNT(*) FROM albumes") fun contarAlbumesFlow(): Flow<Int>
    ...
    data class AnioEstadisticas(val anio: Int, val cantidad: Int)
    ...
    @Query(
        "SELECT id_album FROM albumes WHERE titulo = :titulo AND id_artista = :artistaId LIMIT 1"
    )
    ...
    suspend fun obtenerOCrearAlbum(titulo: String, artistaId: Int, anio: Long?): AlbumEntity {
        ...
        return existente
            ?: run {
                val nuevoAlbum = AlbumEntity(titulo = titulo, idArtista = artistaId, anio = anio)
