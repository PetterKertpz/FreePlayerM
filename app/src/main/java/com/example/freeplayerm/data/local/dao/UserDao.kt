...
interface UserDao {
   ...
   /**
    * Inserta un nuevo usuario Falla si ya existe un usuario con el mismo correo o nombre de usuario
    *
   ...
   suspend fun obtenerUsuariosActivosRecientes(
      timestamp: Long = System.currentTimeMillis() - (7L * 24 * 60 * 60 * 1000) // ✅ Long
   ): List<UserEntity>
   ...
   @Query("SELECT COUNT(*) FROM usuarios WHERE activo = 1") suspend fun contarUsuariosActivos(): Int
   ...
   /**
    * ⚠️ DEPRECADO: Usar UserRepository.login() en su lugar. La verificación BCrypt no puede hacerse
    * en SQL/DAO.
    */
   @Deprecated(
   ...
   suspend fun limpiarUsuariosInactivosAntiguos(
      timestamp: Long = System.currentTimeMillis() - (90L * 24 * 60 * 60 * 1000) // ✅ Long
   ): Int
   ...
   @Query(
      "UPDATE usuarios SET total_reproducciones = total_reproducciones + 1 WHERE id_usuario = :userId"
   )
   suspend fun incrementarReproducciones(userId: Int)

   @Query("UPDATE usuarios SET ultima_sesion = :timestamp WHERE id_usuario = :userId")
   suspend fun actualizarUltimaSesion(userId: Int, timestamp: Long)

   // ========== QUERY REACTIVA CON ESTADÍSTICAS CALCULADAS ==========
   ...
   @Query(
      ...
      SELECT
         ...
         u.fecha_creacion,
         u.tema_oscuro,
         u.notificaciones_habilitadas,
         u.reproduccion_automatica,
         u.calidad_preferida,
         u.idioma_preferido,
         u.total_reproducciones,
         COALESCE((SELECT COUNT(*) FROM favoritos WHERE id_usuario = u.id_usuario), 0) as total_favoritos,
         COALESCE((SELECT COUNT(*) FROM listas_reproduccion WHERE id_usuario = u.id_usuario), 0) as total_listas
      FROM usuarios u
      WHERE u.id_usuario = :userId AND u.activo = 1
   """
   )
   fun obtenerUsuarioPorIdFlow(userId: Int): Flow<UserEntity?>
}
