// en: app/src/main/java/com/example/freeplayerm/core/security/SecurityHelper.kt
...
object SecurityHelper {
     ...
     * @return Un String que representa el hash de la contraseña. Este es el valor que guardaremos
     *   en la base de datos.
     */
     ...
     *
     * En producción, considera usar JWT (JSON Web Tokens) para tokens más seguros que pueden
     * contener información firmada.
     *
    ...
    fun generarTokenSesion(usuarioId: Int): String {
        ...
        val userSalt =
            (usuarioId * 31 + System.currentTimeMillis())
                .toString()
                .hashCode()
                .toString(16)
                .padStart(8, '0')
     ...
     *
     * Los refresh tokens suelen ser más largos y tienen vida más larga Se usan para obtener nuevos
     * access tokens sin re-autenticación
     *
     * @param usuarioId ID del usuario
     ...
     *
     * Útil si decides usar un algoritmo diferente a BCrypt (BCrypt genera salt automáticamente)
     *
    ...
    /**
     * Sanitiza un string para prevenir inyección SQL (Room ya protege contra esto, pero útil para
     * casos especiales)
     *
    ...
    data class ResultadoValidacion(val valido: Boolean, val mensaje: String)
