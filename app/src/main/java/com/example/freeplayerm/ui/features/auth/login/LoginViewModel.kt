        ...
        viewModelScope.launch {
            when (resultadoSignIn) {
                    ...
                    repositorio
                        ...
                        .onSuccess { usuario ->
                            if (usuario.tokenSesion == null) {
                                android.util.Log.e(
                                    "LoginViewModel",
                                    "Token de sesión es null después de buscarOCrearUsuarioGoogle para correo: ${datosUsuario.correo}",
                                )
                                _estadoUi.update {
                                    it.copy(
                                        cargandoConGoogle = false,
                            ...
                            _estadoUi.update {
                                it.copy(
                                    cargandoConGoogle = false,
                                    usuarioIdExitoso = usuario.idUsuario,
                                )
                            }
                        }
                        .onFailure { error ->
                            ...
                            android.util.Log.e(
                                "LoginViewModel",
                                "Error en buscarOCrearUsuarioGoogle: ${error.message}",
                                error,
                            )
                            _estadoUi.update {
                                it.copy(
                ...
                is SignInResult.Error -> {
                    android.util.Log.e(
                        "LoginViewModel",
                        "SignInResult.Error: ${resultadoSignIn.message}",
                    )
                    _estadoUi.update {
                        it.copy(error = resultadoSignIn.message, cargandoConGoogle = false)
                    }
                }
                is SignInResult.Cancelled -> {
                    android.util.Log.d(
                        "LoginViewModel",
                        "Login con Google cancelado por el usuario",
                    )
        ...
        viewModelScope.launch {
            repositorio
                ...
                .onSuccess { usuario ->
                    ...
                    _estadoUi.update {
                        it.copy(cargandoLocalmente = false, usuarioIdExitoso = usuario.idUsuario)
                    }
                }
                .onFailure { error ->
