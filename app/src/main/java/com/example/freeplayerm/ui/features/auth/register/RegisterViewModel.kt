...
constructor(
    ...
    fun enEvento(evento: RegistroEvento) {
        when (evento) {
            is RegistroEvento.NombreUsuarioCambiado ->
                _estadoUi.update { it.copy(nombreUsuario = evento.valor, error = null) }
            is RegistroEvento.CorreoCambiado ->
                _estadoUi.update { it.copy(correo = evento.valor, error = null) }
            is RegistroEvento.ContrasenaCambiada ->
                _estadoUi.update { it.copy(contrasena = evento.valor, error = null) }
            is RegistroEvento.ConfirmarContrasenaCambiada ->
                _estadoUi.update { it.copy(confirmarContrasena = evento.valor, error = null) }
            RegistroEvento.BotonRegistroPresionado -> registrarseLocalmente()
            RegistroEvento.ConsumirEventoDeNavegacion ->
                _estadoUi.update { it.copy(usuarioIdExitoso = null) }
    ...
    private fun registrarseLocalmente() {
        ...
        if (
            estado.nombreUsuario.isBlank() || estado.correo.isBlank() || estado.contrasena.isBlank()
        ) {
        ...
        viewModelScope.launch {
            repositorio
                ...
                .onSuccess { usuarioCreado ->
                    ...
                    _estadoUi.update {
                        it.copy(estaCargando = false, usuarioIdExitoso = usuarioCreado.idUsuario)
                    }
                }
                .onFailure { error ->
                    _estadoUi.update {
                        it.copy(estaCargando = false, error = error.message ?: "Error al registrar")
                    }
                }
        }
    }
