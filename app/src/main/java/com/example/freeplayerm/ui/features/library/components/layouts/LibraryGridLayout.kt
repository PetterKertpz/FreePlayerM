...
fun <T : LibraryItem> LibraryGridLayout(
   ...
   val dynamicHorizontalSpacing by
      animateDpAsState(
   ...
   Box(
            ...
            awaitEachGesture {
               ...
               do {
                  ...
                  if (pointers.size == 2 && !gestoProcesado) {
                     ...
                     val currentDistance = calculateDistance(pointer1.position, pointer2.position)

                     val previousDistance =
                        calculateDistance(pointer1.previousPosition, pointer2.previousPosition)
      ...
      when {
         ...
         items.isEmpty() -> {
            ...
            // 📐 Número de columnas según nivel de zoom
            val columnas = LibraryZoomConfig.columnasGrid(nivelZoom)
...
private fun PreviewLibraryPopulated(
   ...
   FreePlayerMTheme(darkTheme = true) {
      LibraryGridLayout(
         ...
         Box(
            modifier =
               Modifier.aspectRatio(1f).background(Color(item.color), RoundedCornerShape(12.dp)),
